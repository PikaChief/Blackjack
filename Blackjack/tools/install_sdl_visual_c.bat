...
rem 1. it is located your project's "tools" (or as another name) directory, 
...
rem    directory, which itself is under the solution's directory (and 
...
rem Detecting Environment and Platform
if not exist devenv.exe (
    echo This batch script must be run from Visual Studio's 
    echo "Developer Command Prompt for VS XXXX" or as a Visual Studio
    echo Pre-Built Event within the project's properties with the 
    echo command line:
    echo.    call "$(ProjectDir)tools\install_sdl_visual_c.bat"
    exit 7
) else (
    echo devenv.exe detected. Continuing...
)
...
if exist "%sdl_dir%\include\" (
    set _git_submod=T
    echo SDL git submodule installed!
) else (
